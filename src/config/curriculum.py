...
# Curriculum: 200 topics x 10 LOs, with DAG edges
NUM_TOPICS = 200
NUM_LOS = 10
...
def generate_curriculum():
    ...
    for topic in range(1, NUM_TOPICS + 1):
        for lo in range(1, NUM_LOS + 1):
            ...
            # Prerequisites: for LO1, none; for others, previous LO in same topic
            if lo == 1:
                prereq = ''
            else:
                prereq = f"T{topic}_LO{lo-1}"
            # Randomly add cross-topic edges (simulate DAG, not cyclic)
